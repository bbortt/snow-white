package io.github.bbortt.snow.white.rest.utils;
...
/**
 * Originates from <a href="https://stackoverflow.com/a/62624820/9031541">this beautiful
 * StackOverflow</a> post.
 */
public class MultipartParser {
  ...
  /**
   * A form field which stores the field or file data completely in memory. Will be limited by the
   * maximum size of a byte array (about 2GB).
   */
  ...
  /**
   * Assumes the request body really is multipart/form-data. Given the binary request body and the
   * Content-Type header value, attempts to parse fields into a map from field name to list of
   * FileItem objects.
   * <p>
   * Everything is stored in memory and an individual item will only be limited by the maximum
   * size of a byte array (about 2GB). It is recommended that the user sets a limit on maximum
   * upload request size. Doing this will obviously differ by environment.
   * <p>
   * Example:
   * <code>
   * var fields = MultipartParser.parseRequest(requestBody, contentTypeHeader); String firstName =
   * fields.get("firstname").get(0).getString(); byte[] profilePic =
   * fields.get("picture").get(0).get();
   * </code>
   *
   ...
   */
  public static Map<String, List<FileItem>> parseRequest(byte[] requestBody, String contentTypeHeader) throws FileUploadException {
