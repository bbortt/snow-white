...
describe('API Index API', () => {
  describe('Deployment', () => {
    const renderAndGetDeployment = async (manifests?: any[]) => {
      ...
      const deployment = manifests.find(
        (m) =>
          m.kind === 'Deployment' &&
          m.metadata.name === 'snow-white-api-index-api-test-release',
      );
    ...
    it('should be kubernetes Deployment', async () => {
      ...
      expect(metadata.name).toMatch('snow-white-api-index-api-test-release');
    });
    ...
    it('should truncate long release name', async () => {
      ...
      const deployment = manifests.find(
        (m) =>
          m.kind === 'Deployment' &&
          m.metadata.name ===
            'snow-white-api-index-api-very-long-test-release-name-that-excee',
      );
    ...
    describe('containers', () => {
      it('should have only one (api-index-api)', async () => {
        const templateSpec = getPodSpec(await renderAndGetDeployment());

        const { containers } = templateSpec;
        expect(containers).toHaveLength(1);

        expect(containers[0].name).toBe('api-index-api');
      });

      describe('api-index-api', () => {
        const renderAndGetApiIndexApiContainer = async (manifests?: any[]) => {
          const templateSpec = getPodSpec(
          ...
          it('should adjust the image tag from values', async () => {
            ...
            const apiIndexApi = await renderAndGetApiIndexApiContainer(
              await renderHelmChart({
                ...
                values: {
                  snowWhite: {
                    apiIndexApi: { image: { tag: customTag } },
                  },
                },
              }),
            );
            ...
            expect(apiIndexApi.image).toBe(
              'ghcr.io/bbortt/snow-white/api-index-api:custom.tag',
            );
          });
        });
        ...
        describe('imagePullPolicy', () => {
          ...
          it('should adjust the image pull policy from values', async () => {
            ...
            expect(apiIndexApi.imagePullPolicy).toBe(imagePullPolicy);
          });
        });
        ...
        describe('env', () => {
          ...
          it('should calculate jdbc connection string', async () => {
            ...
            const springDatasourceUrl = apiIndexApi.env.find(
              (env) => env.name === 'SPRING_DATASOURCE_URL',
            );
            expect(springDatasourceUrl).toBeDefined();

            expect(springDatasourceUrl.value).toBe(
              'jdbc:postgresql://test-release-postgresql.default.svc.cluster.local.:5432/api-index-api',
            );
          });
          ...
          it('should calculate spring datasource password', async () => {
            ...
            const apiIndexApi =
              await renderAndGetApiIndexApiContainer(manifests);

            const springDatasourcePassword = apiIndexApi.env.find(
          ...
          it('should accept additional environment variables', async () => {
            ...
            const fooEnv = apiIndexApi.env.find((env) => env.name === 'foo');
        ...
        describe('with postgresql disabled', () => {
          it('should fail without defined SPRING_DATASOURCE_URL environment variable', async () => {
            await expect(
              ...
              "Required environment variable 'SPRING_DATASOURCE_URL' is missing in snowWhite.apiIndexApi.additionalEnvs",
            );
          });
          ...
          it('should fail without defined SPRING_DATASOURCE_USERNAME environment variable', async () => {
            await expect(
              ...
              "Required environment variable 'SPRING_DATASOURCE_USERNAME' is missing in snowWhite.apiIndexApi.additionalEnvs",
            );
          });
          ...
          it('should fail without defined SPRING_DATASOURCE_PASSWORD environment variable', async () => {
            await expect(
              ...
              "Required environment variable 'SPRING_DATASOURCE_PASSWORD' is missing in snowWhite.apiIndexApi.additionalEnvs",
            );
          });
        });
      });
  ...
  describe('Service', () => {
    const renderAndGetApiIndexApiService = async (manifests?: any[]) => {
      if (!manifests) {
        manifests = await renderHelmChart({
          chartPath: 'charts/snow-white',
        });
    ...
    describe('selector', () => {
      ...
      it('should select correct pod based on selector labels', async () => {
        ...
        const service = await renderAndGetApiIndexApiService(manifests);
