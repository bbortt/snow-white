...
In this example not even generated code does: the API is produced by the Snow-White OpenAPI generator from [`specs/ping-pong.yml`](./specs/ping-pong.yml), and the generator contributes nothing to the copy either.
...
./mvnw -Pprod -DskipTests -pl :example-snow-white-openapi-generator -am clean install
docker build -t ghcr.io/bbortt/snow-white/example-snow-white-openapi-generator:local \
  examples/example-snow-white-openapi-generator

docker compose \
  -f examples/example-snow-white-openapi-generator/src/apptest/resources/docker-compose-apptest.yaml \
  up -d --wait

./mvnw -Papptest -Ddocker.network=github_actions -Dimage.tag=local \
  -pl :example-snow-white-openapi-generator verify
```
