...
An application test drives each example application — `examples/example-spring-boot`, whose API is
hand-written, and `examples/example-snow-white-openapi-generator`, whose API is generated, both
instrumented by the OpenTelemetry Java agent with
`otel.java.experimental.span-attributes.copy-from-baggage.include` set — with an HTTP request
carrying a `baggage: test.case.name=<name>` header, and asserts the exported server span carries
the configured attribute with that value, having added no application code to either example.
Covering both integration stories is what makes the `STK-003` claim a property of the agent
configuration rather than of one author's restraint.
...
- **2026-09-26** — Extended the producing half to the second example application:
  `examples/example-snow-white-openapi-generator` now sets the same agent property in its image and
  carries the same application test.
  The first example's API is hand-written, so "no application
  code takes part" could be read as the author having simply written none; this one's
  request-handling layer is generated by the Snow-White OpenAPI generator, and the attribute still
