...
class OtelEventFilterStreamAppTest {
  ...
  void shouldDropInboundEventWhenApiIndexServiceIsPersistentlyUnavailable(
    ...
    // The outbound topic is confirmed empty as soon as the first poll comes back empty, which
    // can race ahead of the service's own retry backoff (200ms + 400ms between the 3 attempts).
    // Poll the WireMock verification until it settles instead of asserting immediately.
    verifyEventually(
      3,
      getRequestedFor(urlPathTemplate(EXISTS_PATH_TEMPLATE))
        .withPathParam("serviceName", equalTo(OTEL_SERVICE_NAME))
